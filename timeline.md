...
## 2026-09-02 13:32 PDT — First probe run exposed a contradictory instruction

Fresh Flights run `42a4a934-a529-4497-b6c8-5437a9462f11` used the correct
combined recording and the previously accepted candidate boundaries. The new
probe worked as intended for Location: its compiler tested a request before
writing the parser, saw a real Portland autocomplete response, corrected the
request identifier, and tested the correction again. Search also started with
a request-only artifact and invoked the probe before parser work. No playbook
was used.

Calendar exposed a prompt defect. Its first three probes all failed before
returning a response: first because fresh page state was not captured, then
twice because a locally written date-validation expression was escaped
incorrectly. The compiler eventually fixed that local expression and its
offline tests passed, but it did not test the repaired request. It declared the
artifact mechanically complete and handed it to the master anyway. The run was
cancelled immediately rather than allowing the master to rediscover that
request bug through the expensive verification loop.

The cause was two conflicting instructions. The API-probe section said to
write a parser only after a credible response, while a later generic failure
section allowed unresolved attempts to be handed downstream. The prompt now
has one clear rule: dry-run the request transform and its offline request test
before a browser-backed probe; after a failed probe is repaired, probe again;
and do not claim an ordinary safe API request is complete without a credible
response. If the accepted plan truly lacks a live value or producer, report
that exact gap to the master. Recording-only completion remains available only
when observed rate limiting or bot protection makes more compiler calls
counterproductive. This is an agent instruction, not a new runtime gate or a
site-specific rule.
