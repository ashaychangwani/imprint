...
| **Generated module** | The TypeScript file emitted by `imprint emit` — a thin wrapper over the local `runtime.executeWorkflow` with the workflow inlined. Committable, but rerun `imprint emit <workflow.json> --force` after moving it to another machine so the runtime import matches that install. |
