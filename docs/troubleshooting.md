...
- **INCONCLUSIVE** — there was not enough complete audit evidence for a trustworthy pass. This includes zero gradeable units (for example, every call was `infra`), a connected tool with no invocation attempt, or an advertised parameter with no explicit verdict. The report's `missingTools`, `missingParameters`, and invocation reasons distinguish these cases. Infrastructure-only runs are not code failures; re-run them (often from a different network) and spot-check that a real bug was not mislabeled.
...
- **PASS** — `score ≥ min-score` AND at least `max(1, gradeableTools)` gradeable units, with every connected tool attempted and every advertised parameter given an explicit verdict. A gradeable unit may be a core invocation verdict or a parameter verdict; `gradeableTools` counts tools with at least one such unit. The floor is one gradeable unit per gradeable tool. A tool the auditor could not grade is still listed under `ungradeableTools`, while a tool it never attempted makes the run inconclusive.
