...
- **PASS** — `score ≥ min-score` AND at least `2 × toolCount` gradeable invocations. The `≥ 2 × toolCount` floor ensures the number is backed by enough signal; a site with only one graded call per tool reads as `fail` even at 100%.
