...
**Authenticate tools whose login runs on the `playbook` rung** (sites whose login can't be replayed as API requests — the page mints its credential POST per session) drive a real local browser that fills the login form with your stored credentials and submits it over TLS — the same bytes your own browser would send. Credentials stay placeholders (`${credential.*}`) in the saved `playbook.yaml`; the real values are read from the credential backend at runtime and are never written to the artifacts, logs, or any LLM prompt. The resulting session cookies are persisted back to the credential store for the site's data tools. (Diagnostic screenshots, written only to a temp dir on failure and never committed, can show a filled form — treat them as sensitive.) Playbook browsers close at the end of each run.
