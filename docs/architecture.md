...
**Authenticate-tool backends.** Unlike data tools (which walk the cheap-first ladder), an `authenticate` tool verifies first on **headed `cdp-replay`** — a real, visible Chrome. Auth is pinned away from the closed-browser rungs (`fetch`/`fetch-bootstrap`/`stealth-fetch`) because a login behind a behavioral anti-bot edge only passes from a live browser: those rungs are edge-403'd on the protected credential POST, *headless* Chrome is fingerprinted beyond the `HeadlessChrome` UA token we strip (so `runCdpReplay` launches **headed** for `toolKind === 'authenticate'`), and only cdp-replay holds one live session open across the two 2FA phases. There is no bespoke login backend. For browser-minted credential bodies that cannot be replayed far enough to produce the 2FA challenge, auth may include a constrained sibling `playbook.yaml` for the `initiate` phase; the verifier still tries cdp-replay first and falls back to that playbook only on pre-challenge failures. `executeAuthWorkflow` (`runtime.ts`) adds the auth phasing: it splits the recorded `requests` into `initiate` / `complete` / `submit_otp` by `initiateRequestCount`, polls `pollEndpoint` for push approval, returns `AWAITING_2FA` for 2FA types, and skips a non-2xx on an `optional: true` request (a best-effort "remember device"/telemetry step that must not block the login).

**Auth compile is an agent-orchestrated, segmented loop** (`auth-compile-agent.ts`, `auth-verifier.ts`). The compile agent is the *brain*: it shapes `workflow.json` **from the recording only — it never logs in itself**; for stale browser-minted credential submits it may also emit a constrained login `playbook.yaml` for initiate. A separate **verification stage** is the only thing that fires a live login. The agent pauses at **checkpoint tools** (`run_verification` / `prompt_user` / `wait_for_cooldown`); each writes a sentinel and ends the agent's turn (claude-cli/codex-cli segment), and the orchestrator (`teach`) performs the action and **resumes the same CLI session** with the result. `AuthVerifier` keeps the live session continuous across 2FA phase 1 (send the OTP/push → `AWAITING_2FA`) → the user's input (`prompt_user` renders an agent-generated message in the teach TUI) → phase 2 (submit the code / poll). It runs phases on headed `cdp-replay` (`forceBackend: 'cdp-replay'`): the **persistent `cdpPool`** holds the same live browser across the user-input gap (the `AWAITING_2FA` carve-out in `runCdpReplay` retains it), and the challenge token echoed on `AWAITING_2FA` is threaded back as `initialState` so the completion's `${state.X}` resolves in the *exact* session that requested the 2FA. If cdp-replay fails before delivering a challenge and a sibling login playbook exists, `AuthVerifier` retries initiate through `playbook` so the page can mint a fresh credential body; it does not use playbook for ordinary replayable auth or for the completion poll. Pinning cdp-replay first (rather than walking the ladder) is deliberate: only a live **headed** browser passes a behavioral-anti-bot login at all (the closed-browser rungs are edge-403'd on the protected credential POST, and headless is fingerprinted past the UA token), and cdp-replay keeps one session open across both phases — left to the probe, a stateless rung like `fetch` could "win" `initiate` on leftover cookies and then 401 the completion poll with "tokens missing". The single live `initiate` per try fires at most one OTP/push. Live `initiate` is bounded by **two budgets**: a **challenge** cap (`IMPRINT_AUTH_MAX_INITIATE`, default 2) counts only initiates that actually *delivered* a 2FA prompt (`AWAITING_2FA` or a completed login), so the user sees at most two prompts; an **attempt** cap (`IMPRINT_AUTH_MAX_INITIATE_ATTEMPTS`, default 5) bounds total tries so a login blocked *before* any challenge (e.g. an edge 403 — which sent nothing) can't loop forever. A pre-challenge failure therefore does **not** burn the challenge budget, so a corrected workflow can still be verified in the same run. On a rate-flag the agent calls `wait_for_cooldown` (5–10 min, no login) instead of hammering. Because the verifier reaches the login through `cdp-replay`'s live page, an auth `workflow.json` carries a top-level **`bootstrap.url` = the credential-entry page** (the page the recording navigated to right before the credential POST): the rung navigates it first so the login page's anti-bot sensor runs and validates its token (e.g. Akamai `_abck`) for the correct Origin — otherwise the credential POST is edge-blocked (403) before reaching the 2FA step. Auth also runs on a **clean cookie slate**: cdp-replay seeds a cached jar/recording's cookies into the fresh browser for *data* tools (to reuse an authed session) but **never for an `authenticate` tool** — a login establishes a brand-new session, and seeding a prior run's stale anti-bot cookie (the page revalidates a fresh `_abck` to `~0~`, but the seeded stale one gets the credential POST edge-403'd) would otherwise re-arm itself every attempt, since `saveJar` persists a jar after each run. The compile agent sets it; if it doesn't, the orchestrator derives it deterministically from the recording (`auth-bootstrap.ts`: the credential POST's `Referer` → the form-hosting Document → the last Document before the POST). The teach spinner surfaces a cumulative turn count across resumed segments and, when a live verification fails, the reason inline (`verify initiate FAILED (FORBIDDEN HTTP 403); attempt 2/5 — agent retrying`). A completed login persists a **durable session token** — cookies plus any `authConfig.sessionCapture` values (a bearer/CSRF token from the completion response, stored as `${credential.NAME}`) — so data tools reuse it and re-auth only on `AUTH_EXPIRED`. Two per-site DATA patterns the compile agent emits for a robust 2FA tool (`prompts/auth-compile-agent.md`): **predicate capture paths** (`challengeQuestions[category=PUSH_NOTIFICATION].…` — select an array element by a discriminator field, not a fixed `[0]`, since a site lists its 2FA methods in non-deterministic order) and the **`optional: true`** request flag (a best-effort step like "remember this device" that may 4xx without aborting the login — `executeAuthWorkflow` logs and skips it).
...
- **Per-request-nonce login** — a value the page must mint *for this exact POST* (a one-time WebCrypto challenge, a per-load reCAPTCHA, or a per-load encrypted credential envelope) is attempted first through cdp-replay. If cdp-replay cannot reach the recorded challenge response and the recording contains the actual credential-entry DOM actions, auth may add a constrained `playbook.yaml` initiate fallback so the page mints the fresh body. It still fails honestly if the page hard-blocks automation or the required challenge/session tokens cannot be captured; no success marker is weakened.

**One live session across both phases.** On the primary cdp-replay path, `initiate` and `complete`/`submit_otp` run on the SAME pooled headed Chrome (`cdpPool`), so the page's in-page JS state — live WebCrypto handles, closures, a single-use in-page token — survives the user-input gap without serialization. If initiate falls back to a browser-minted login playbook, phase 2 does not reuse that exact page; it carries only serializable state (persisted cookies/localStorage plus `twoFactorContext` captures), so the fallback is reserved for credential submits whose fresh page-minted body is the load-bearing part. Body-returned challenge tokens (a reauth `mfaId`) ride `twoFactorContext` → `initialState`; cookies persist via `saveSiteCookies`. `otp` spans two calls (`initiate` → `submit_otp` with `otp_code`, plus the echoed `twoFactorContext`); `push`/`none` complete in one `initiate` (`push` blocks on `complete`). In `--no-interactive` teach the completion is **attempted unattended** — a placeholder `otp_code` (`000000`) for `otp`, a bounded poll for `push` while the user approves on their phone — and the outcome is reported honestly.
