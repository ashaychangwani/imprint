...
**Auth compile is an agent-orchestrated, segmented loop** (`auth-compile-agent.ts`, `auth-verifier.ts`). The compile agent is the *brain*: it shapes `workflow.json` (+ `playbook.yaml` for browser-minted logins) **from the recording only — it never logs in itself**. A separate **verification stage** is the only thing that fires a live login. The agent pauses at **checkpoint tools** (`run_verification` / `prompt_user` / `wait_for_cooldown`); each writes a sentinel and ends the agent's turn (claude-cli segment), and the orchestrator (`teach`) performs the action and **resumes the same claude session via `--resume`** with the result. `AuthVerifier` holds one **persistent `cdpPool`** so the live session is reused across 2FA phase 1 (send the OTP/push → `AWAITING_2FA`) → the user's input (`prompt_user` renders an agent-generated message in the teach TUI) → phase 2 (submit the code / poll) — never a fresh browser, which would reset the challenge. Live `initiate` is bounded by **two budgets**: a **challenge** cap (`IMPRINT_AUTH_MAX_INITIATE`, default 2) counts only initiates that actually *delivered* a 2FA prompt (`AWAITING_2FA` or a completed login), so the user sees at most two prompts; an **attempt** cap (`IMPRINT_AUTH_MAX_INITIATE_ATTEMPTS`, default 5) bounds total tries so a login blocked *before* any challenge (e.g. an edge 403 — which sent nothing) can't loop forever. A pre-challenge failure therefore does **not** burn the challenge budget, so a corrected workflow can still be verified in the same run. On a rate-flag the agent calls `wait_for_cooldown` (5–10 min, no login) instead of hammering. Because the verifier runs the login inside `cdp-replay`'s live browser, an auth `workflow.json` carries a top-level **`bootstrap.url` = the credential-entry page** (the page the recording navigated to right before the credential POST): cdp-replay navigates it first so the login page's anti-bot sensor runs and validates its token (e.g. Akamai `_abck`) for the correct Origin — otherwise the credential POST is edge-blocked (403) before reaching the 2FA step. The compile agent sets it; if it doesn't, the orchestrator derives it deterministically from the recording (`auth-bootstrap.ts`: the credential POST's `Referer` → the form-hosting Document → the last Document before the POST). The teach spinner surfaces a cumulative turn count across resumed segments and, when a live verification fails, the reason inline (`verify initiate FAILED (FORBIDDEN HTTP 403); attempt 2/5 — agent retrying`). A completed login persists a **durable session token** — cookies plus any `authConfig.sessionCapture` values (a bearer/CSRF token from the completion response, stored as `${credential.NAME}`) — so data tools reuse it and re-auth only on `AUTH_EXPIRED`.
