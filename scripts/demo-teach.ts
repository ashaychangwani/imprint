 ...
 *   bun scripts/demo-teach.ts --emit-web   # regenerate web/src/data/teach-demo.js
 */
import { writeFileSync } from 'node:fs';

const SPEED = Number(process.env.DEMO_SPEED ?? '1') || 1;
const STATIC = process.argv.includes('--static');
const EMIT_WEB = process.argv.includes('--emit-web');
...
if (EMIT_WEB) {
  const path = new URL('../web/src/data/teach-demo.js', import.meta.url).pathname;
  const banner = '// AUTO-GENERATED by scripts/demo-teach.ts --emit-web. Do not edit by hand.\n';
  const body = `export const teachDemo = ${JSON.stringify(committed, null, 2)};\n`;
  writeFileSync(path, banner + body);
  out(`\nwrote ${path} (${committed.length} lines)\n`);
}
