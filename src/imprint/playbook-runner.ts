...
export async function extractResult(
  ...
  if (result.source === 'xhr') {
    ...
    // `*` returns the full body unchanged (parsed JSON or raw string) —
    // useful when the consumer (parser.ts, MCP caller) wants the rich
    // object graph or needs to decode a custom envelope itself.
    if (result.extract === '*') {
