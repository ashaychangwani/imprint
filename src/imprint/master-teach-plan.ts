...
/**
 * The shipped detector proposes semantics; this handoff grounds its optional
 * event citations in the recording before an advisor sees them. The candidate
 * and every other field remain intact so the master can revise its boundary.
 * Advisor and master output still goes through the normal strict validation.
 */
export function groundDetectorCandidateForMaster(
  ...
  return TeachingToolCandidateSchema.parse({
    ...candidate,
    eventSeqs: candidate.eventSeqs.filter((seq) => recording.eventSeqs.has(seq)),
  });
}
