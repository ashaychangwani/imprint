...
/** Assemble the full auth compile toolset (read/write tools + the two
 *  auth-specific live-test tools). Callers append done()/give_up() themselves
 *  in whatever form their driver needs. */
export function buildAuthCompileTools(
  ...
  return [
    ...
    buildTestAuthWorkflowTool(toolDir, teachCredentials),
  ];
}
...
function buildTestAuthWorkflowTool(toolDir: string, teachCredentials: TeachCredentials): AgentTool {
  return {
    ...
    input_schema: {
      ...
      properties: {
        params: {
          type: 'object',
          description:
            'Parameters to pass to the workflow (e.g. {"action": "initiate"}). Defaults to {"action": "initiate"}.',
        },
      },
      required: [],
    },
    handler: async (input: unknown) => {
      ...
      try {
        const ladderResult = await runWorkflowWithLadder({
          workflowPath,
          params,
          credentials: {
            site: teachCredentials.site,
            cookies: [],
