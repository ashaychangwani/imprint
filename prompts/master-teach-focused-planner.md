...
The artifact still cannot copy an arbitrary XHR generated by page JavaScript
into a later request: it cannot subscribe to, intercept, copy, or mutate that
XHR. It does not need to when the rendered page is already the result.
Navigation is not an implicit pre-step: declare it as the load-bearing workflow
request when its rendered response is the evidence-backed result.
