...
1. `prepare_live_backend` checks whether the workflow already has a valid backend
...
2. `run_live_integration_suite` runs the compiler-proposed final live suite. You
...
3. `run_live_integration_test` makes a targeted live call for a specific unresolved
...
4. `submit_verification_report` submits your structured final judgment. Call it
