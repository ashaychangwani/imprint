...
3. **`request-transform`** — request preparation shared across tools. Wire-up: the consuming tool sets `requestTransformModule: "../_shared/<name>.ts"`. Ground it in exact recorded request/response evidence and `sourceSeqs`. The exported `transform(method, url, responses, params?)` returns either a body string or `{ url?, body?, headers?, navigation?, skip? }`, matching the runtime contract.
...
10. **Be conservative.** Never invent a module without grounding `sourceSeqs`. If unsure whether two tools truly share logic, leave it per-tool (empty `sharedModules`, empty `usesSharedModules`). These are advisory module proposals for the master to consider, not host-enforced import assignments. Fewer, well-grounded modules beat many speculative ones.
11. `paramChecklist` is your evidence-backed proposal for the public inputs worth investigating during focused compilation. It may add, rename, or drop candidate `likelyParams`; the detector list is not authoritative.
12. **Producer-derived chains (`emitsTokens` / `tokenParams`).** When one tool's param is an opaque id/token OR another context value a caller cannot type reliably — because its value is minted/derived by ANOTHER selected tool's response (a `search_*` → `get_*_details` chain) — model it as a cross-tool compatibility target instead of asking the caller/auditor to fabricate it. An `emitsTokens` entry does not redefine the producer's core success: useful producer records and input classes remain valid when an optional downstream token is unavailable, provided compilation records the missing output and affected consumers as a limitation. Start from `tokenContractHints[]` (each entry is a pre-detected edge you MUST declare), and also catch any the diff missed (`ephemeralValues` with a `server_derived` `producerSeq` belonging to a different tool's `requestSeqs`, or a `dependencySeqs` link):
    ...
    - On the PRODUCER (`sourceTool`), add `emitsTokens: [{ field, shape }]` so its parser uses that exact `field` and emits the full shape when the response supplies it (e.g. a composite of id + area context), never a bare or fabricated fragment. If some useful result class lacks the field, preserve that class and require an explicit producer limitation; leave affected consumers limited/unverified rather than narrowing the producer merely to make the chain total.
    - If the consumer will unpack a composite token object/string, `shape` MUST name the exact serialized keys the consumer requires, using `requiredKeys=[keyA,keyB,...]` in the shape text. Do not use vague labels like "context" or "selection metadata" when the consumer needs concrete members; name the concrete keys the consumer transform will read. Example: `shape: "serialized JSON object; requiredKeys=[itemId,variantId,selectedOption,sessionHandle]"`.
    - If a consumer needs multiple sibling fields from the same selected result (for example an item token plus a selected-option descriptor, variant id, quote context, request context, or session-specific handle), declare EACH field as its own `tokenParams` entry and EACH producer field in `emitsTokens`, or declare one composite field whose `shape` lists every required subkey via `requiredKeys=[...]`. Do not declare only the opaque token and leave the rest as free-form user input; those companion fields are just as producer-derived and must be chained from the same producer output.
