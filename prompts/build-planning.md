...
12. **Opaque-token chains (`emitsTokens` / `tokenParams`).** When one tool's param is an opaque id/token a user cannot type — its value is minted by ANOTHER selected tool's response (a `search_*` → `get_*_details` chain) — model it as a cross-tool contract instead of bundling the context into an opaque blob. Start from `tokenContractHints[]` (each entry is a pre-detected edge you MUST declare), and also catch any the diff missed (`ephemeralValues` with a `server_derived` `producerSeq` belonging to a different tool's `requestSeqs`, or a `dependencySeqs` link):
    ...
    - On the PRODUCER (`sourceTool`), add `emitsTokens: [{ field, shape }]` so its parser emits that exact `field` in the full `shape` the consumer needs (e.g. a composite of id + area context), NOT a bare fragment. **Emit the id in the FORMAT the consumer actually consumes.** A resolver response often carries several ids for the same entity (e.g. a `/m/...` Knowledge-Graph mid AND a numeric/`0x...` feature id). The consumer only accepts the one its recorded request carried at the consuming position — inspect that position (`inputProvenanceHints` / the recorded consumer body) and have the producer emit THAT format. Emitting a different id the resolver also returns (e.g. the numeric feature id when the search consumes a `/m/` mid) makes the chain return empty/zero results even though both tools "work" in isolation. The `shape` must name the exact format (e.g. "the `/m/...` KG place id, not the numeric feature tuple").
    ...
    - **Route a resolver's entity id to the PRIMARY search, not just detail tools.** When a resolver/suggest tool emits an entity id (a place/location id, a store/account id) and the recording shows the **primary search** tool's request carries that same id type at a position (e.g. a `/m/...` place id in the search body) — even if the dual-pass diff only linked the resolver to a secondary `get_*_details` tool — ALSO declare the search tool as a consumer (`tokenParams`) of that id. A primary search that exposes only the user's raw **text** for the entity silently defaults at replay time (a text-only location geo-defaults to the *caller's* IP location; a text-only store/account falls back to a global/default scope) and returns confident, well-formed results for the WRONG entity. The id makes the search deterministic across IPs, so the resolver→search edge is the load-bearing one even when the recording happened to resolve the text implicitly via live browser/session context. Keep the raw-text param too (as a convenience default), but the id param is what makes the result correct.
