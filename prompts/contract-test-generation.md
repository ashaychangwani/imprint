...
- If the tool searches for items (flights, hotels, products), assert the response has a top-level array field and it's non-empty for the baseline query.
...
**Use generic paths when the exact parser field name is unknown.** The test runner resolves paths against the actual `result.data` shape. Use descriptive path guesses based on the narration (e.g., `flights`, `outbound_flights`, `results`, `suggestions`, `options`).
...
- Asserts the response reflects the constraint (e.g., `origin_airport=LAX` → origin field contains "LAX")
...
**Filter parameters** (stops, price caps, airline filters): assert the response is *constrained* — every item in the result satisfies the filter.
...
When a parameter is echoed back in the response (common for origin/destination airports, search queries, dates):
- Assert the response contains the parameter value in the expected field.

### 4. Edge cases

- Zero/empty filter values (e.g., `stops=0`, `airlines=""`, `max_price=0`) should produce unfiltered results (non-empty response).
...
- If narration mentions using parameters together (e.g., "one way flight" implies `trip_type=2` + empty `return_date`), test that combination.
...
1. **Specific prices, counts, or time-varying data.** Flight prices change every minute. Assert `price > 0`, not `price === 245`.
2. **Specific item identifiers.** Flight numbers, booking tokens, session IDs change per call.
...
Set `baseParams` to a sensible baseline using the parameter defaults from `likelyParams`. For date parameters, use dates 2-3 months in the future to ensure inventory exists. For location parameters, use common, well-serviced values (major airports, popular cities).
