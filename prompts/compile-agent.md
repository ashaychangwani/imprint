...
5. **Write workflow.json.** Template the request(s):
   ...
   - **Resolved-id params — chain, do NOT pass raw text (see `inputProvenanceHints`).** Some user-facing params are NOT carried in the request as the user's text — the backend needs a resolved opaque id (a place/location Knowledge-Graph mid, an account/object handle, a geo id). The recording proves this: the load-bearing request holds an id at some position whose value **first appears in an EARLIER response**, not in the user's typed text. The session summary surfaces these as `inputProvenanceHints` (each gives the `path`, an `example` value, the consuming `inRequestSeq`, and `mintedByResponseSeq`/`mintedByEndpoint`). For every such position:
     - You MUST obtain the id by chaining the minting request and `capture`-ing it, then template the captured `${state.NAME}` into that position. NEVER freeze the recorded id (it's specific to the recorded entity) and NEVER substitute the param's raw text into an id position (the backend silently ignores it and falls back to a default — e.g. a text-only location geo-defaults to the *caller's* IP city, returning the wrong city's results that still look well-formed).
     ...
     - This is the single most important correctness check for search-by-place tools: a tool that returns rich, well-formed results for the *wrong location* passes a careless audit but is broken. If an `inputProvenanceHint` exists for the location/place position, the text-only encoding is wrong — chain it.
