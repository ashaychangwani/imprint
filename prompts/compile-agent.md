...
5. **Write workflow.json.** Template the request(s):
   ...
   - **Destructive requests.** When the session summary shows `destructive: true` on a request, carry that flag to the matching request in workflow.json by setting `"destructive": true`. A destructive request is one whose re-issuance causes an irreversible outward-facing side effect (placing an order, charging a card, sending a message, deleting a resource). The runtime uses this flag to skip the request during compile-verify and audit — returning a synthetic `{ imprintDryRun: true, skipped: "destructive" }` response — so testing never triggers the real action. Production runs always execute destructive requests normally. When the workflow includes destructive requests, note this in `intent.description` — e.g., "Place a food order at the PANW canteen (includes a destructive order-placement step that is skipped during testing)."
...
10. **Write integration.test.ts.** Create a live API test that imports the generated tool and calls it through the backend ladder. This verifies the workflow produces real data — not just that the parser handles recorded responses.
    ...
    **Destructive requests in integration tests.** When the workflow contains requests marked `destructive: true`, the compile-verify harness sets `IMPRINT_DRY_RUN_DESTRUCTIVE=1` in the test environment. This causes the runtime to return a synthetic `{ imprintDryRun: true, skipped: "destructive" }` response for those requests instead of issuing them live. Your integration test should:
    - Still call `runWorkflowWithLadder` and assert `result.ok === true` — the workflow must complete structurally even when destructive requests return synthetic data. The non-destructive parts of the chain (login, auth, search) are verified live.
    - If the destructive request IS the load-bearing request (the one whose response the parser extracts from), the parser will receive the synthetic dry-run response during the integration test. In this case, `parser.test.ts` (which runs against the RECORDED real response) is the primary verification signal for parsing correctness. The integration test validates the auth and navigation chain up to the destructive step.
    - Do NOT skip writing `integration.test.ts` for a tool with destructive requests. The non-destructive prerequisite chain still needs live verification.
...
WorkflowRequest = {
  ...
  destructive?: boolean;    // irreversible side effect — skipped during compile-verify
}
