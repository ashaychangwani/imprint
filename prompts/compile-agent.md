...
You will produce three artifacts in the generated tool directory (`~/.imprint/<site>/<toolName>/` by default). Do **not** write `playbook.yaml` during normal data-tool compilation; the default compile policy is API workflow only.
...
5. **Write workflow.json.** Template the request(s):
   ...
   - **Volatile ID headers are not static config.** Headers whose names contain session/correlation/request/trace/nonce/idempotency semantics (for example `x-request-id`, `*-correlation-id`, `*-api-session-id`) must be wired from `${generated.uuid}` / `${generated.nonce}` or captured state when the site requires an earlier producer. Never replay the recorded literal just because it was present on every captured API call.
   ...
   - **Complex body construction via requestTransformModule.** When the API uses a body format where simple `${param.X}` placeholder substitution cannot correctly encode values — e.g., JSPB arrays in form-encoded fields, nested JSON strings with position-dependent escaping — write a `requestTransformModule` that constructs the body programmatically. The transform receives `params` as a 4th argument and can return an object instead of a string:
     ...
     ```
     Returning a plain `string` (just the URL) still works for simple URL-signing. Use the object return when you need to build or modify the request body or headers. Use `{ skip: true }` for conditional follow-up requests that should only run for some parameter values — for example, run a resolver/base search first, then skip the pagination/detail request when `page <= 1` or no selector is present. Do NOT duplicate a load-bearing API call just because workflow requests are static; if a later request is conditional, put the branch in `request-transform.ts` and skip the request that is not needed. Do NOT invent URL query parameters as a workaround for body-encoding complexity — the server ignores unknown query params and the parameters will have no effect.
...
| `write_file` | Write workflow.json, parser.ts, parser.test.ts, integration.test.ts, request-transform.ts, or notes/*.md in the generated tool directory. Do not create playbook.yaml. |
| `read_file` | Read a file by relative path (e.g. `parser.ts`, `workflow.json`) |
| `run_bash` | Run a shell command (60s timeout, output truncated to 16KB). cwd is the tool directory. Do not use it to create playbook.yaml. |
