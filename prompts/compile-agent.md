...
5. **Write workflow.json.** Template the request(s):
   ...
   - **Resolved-id params — chain the minting request, do NOT pass raw text (see `inputProvenanceHints`).** Some user-facing inputs are NOT carried in the load-bearing request as the user's text — the backend keys off a resolved opaque id (an entity/object handle, an account id, a place/geo id, a category token). The recording proves which: the request holds a value at some position that **first appears in an EARLIER response**, not in anything the user typed. `read_session_summary` surfaces these as `inputProvenanceHints` (each gives the `path`, an `example` value, the consuming `inRequestSeq`, and `mintedByResponseSeq`/`mintedByEndpoint`). For every such position:
     - NEVER freeze the recorded id (it's specific to the recorded entity), and NEVER substitute the param's raw text into an id position — the backend typically ignores an unrecognized value and silently falls back to a default (an unfiltered/global result set, or a server-chosen default scope), so the call returns results that look well-formed but answer the wrong query.
     - **If a SIBLING resolver tool produces this id (the common case — `read_build_plan`'s `tokenParams` lists it, or a `suggest_*`/`autocomplete`/`get_*_details` tool in this recording emits it): do NOT build an internal multi-request chain.** Keep the tool a SINGLE load-bearing request, expose the id as its own `${param.NAME}` parameter (e.g. `place_id`), and template it at the id position — alongside the tool's other params (filters, dates, sort) in the SAME request. The caller (and the audit) mints the id once from the sibling resolver and passes it in. This is critical: wrapping the search in an internal resolve→refine chain tends to break the other params (filters stop applying) — the single-request form keeps every param working AND resolves the entity. Declare the `tokenParams` contract so the chain is verified end-to-end.
     - **`selfChain: true` AND no sibling resolver exists** means the id is minted by the tool's OWN endpoint with no separate resolver tool: only then use *resolve-then-refine* — request[0] carries the user's text, `capture` the resolved id from its response, request[1] is the load-bearing call with `${state.NAME}` at the id position.
     - Treat this as a hard correctness check: a tool that returns rich, well-formed results for the *wrong entity* passes a shallow test but is broken. If an `inputProvenanceHint` (or a `tokenParams` contract) covers a position, the raw-text encoding there is wrong — expose the id param and template it there.
...
10. **Write integration.test.ts.** Create a live API test that imports the generated tool and calls it through the backend ladder. This verifies the workflow produces real data — not just that the parser handles recorded responses.
    ...
    **Import conventions**: The runtime lives at `imprint/runtime` (resolved via a symlink at `~/.imprint/node_modules/imprint` → the repo root). Types live at `imprint/types`. During compilation, `index.ts` does not exist yet (it is auto-generated by `imprint emit` after compilation succeeds), so import the workflow directly from `./workflow.json`.
    ...
    Boilerplate — use `runWorkflowWithLadder` so the test dispatches through `runWithLadder` (the same dispatch the MCP server uses at runtime), exercising the fetch → stealth-fetch escalation. The playbook rung is intentionally excluded at this stage because `playbook.yaml` is compiled in a separate later step (`imprint compile-playbook`); only fetch and stealth-fetch are available during integration-test time. The test passes as long as either rung succeeds, so a tool whose fetch path is blocked by Akamai/PerimeterX still verifies end-to-end via stealth-fetch:
    ```typescript
    import { expect, test } from 'bun:test';
