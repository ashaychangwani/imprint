...
4. **Examine the load-bearing request.** Call `read_request` for the exact candidate sequence. Use `inspect_body_structure` when its body has nested or framed structure that is difficult to compare from the redacted request text alone. Start request construction from the complete recorded request template and change only the exact fields whose construction is supported by evidence. Do not rebuild a positional or multiply encoded protocol from a smaller guessed array merely because an authored unit test accepts that guess.
   ...
   literal, compare same-endpoint recordings when available. Candidate scope
   limits the operation you build, not the evidence you may inspect: use
   `search_requests` to find matching calls across the entire combined
   recording, then read examples spread across sessions instead of reasoning
   only from the representative request. For a one-off endpoint, inspect
   nearby calls in the same request family for shared transport fields. If a
   value changes, consider its live producer, a supported generator, omission,
   and whether a recorded value is actually a longer-lived protocol literal.
   Do not assume that every high-entropy value is per-invocation merely because
   it looks opaque.
   ...
   Preserve each hypothesis as one coherent construction. Coherent does not
   mean every field has the same lifetime: real requests commonly combine
   fresh session state, a newly generated per-call value, and a stable recorded
   protocol literal. Do not respond to an omit-all failure by mechanically
   adding unrelated recorded values one at a time, but do test the strongest
   evidence-backed mixed-lifecycle combinations before declaring the API
   exhausted. State what each live result isolated for the master. One failed
   generator shape does not disprove other shapes still supported by the
   recording or the available fresh-value mechanisms.
...
5. **Write workflow.json.** Template the request(s):
   ...
   - **Complex body construction via requestTransformModule.** When the API uses a body format where simple `${param.X}` placeholder substitution cannot correctly encode values — e.g., JSPB arrays in form-encoded fields, nested JSON strings with position-dependent escaping — write a `requestTransformModule` that constructs the body programmatically. The transform receives `params` as a 4th argument and can return an object instead of a string:
     ...
     prioritized set of meaningfully different constructions across retained
     repair turns. Include the closest recorded construction
     ...
     idea from a disproven one. Continue while distinct evidence-backed
     constructions remain and the shared run deadline allows it.

     Prefer a direct authored API request. When pre-planning research instead
     proved that the parameterized result page itself returns the core data,
...
**Irreversible workflows are never live-tested.** Do not call `runWorkflowWithLadder`, direct fetch, curl, browser actions, or any generated MCP tool for a workflow containing `effect: "irreversible"`. Use recorded response fixtures for parser tests and offline request rendering for parameter fidelity. The host records live verification as not applicable; it never fabricates a successful response.
    ...
    **Proposed producer-consumer values.** A build-plan slice may suggest that one tool emits a field consumed by a sibling. Treat that as an editable hypothesis. Inspect the producer's actual output, the consumer's exact request position, and a fresh chained verification before accepting it. A name such as `token` or a long/opaque shape does not prove origin or lifecycle. Never paste a raw secret into artifacts.
    ...
    Confirm that the workflow's declared result response actually contains every promised producer value. Rendered text does not include a hidden DOM attribute, link URL, or background network value merely because the page used it. If the accepted plan requires a value that no returned response exposes, do not copy a stale recorded value or keep searching the same surface. Report that exact plan contradiction with `give_up` so the master can revise the request graph, representation, or chain edge while retaining this conversation for the corrected plan.

    - **If you accept a PRODUCER suggestion** (`emitsTokens` lists a field), treat it as a downstream compatibility target, not an automatic definition of the producer's core success. Emit the field in the exact shape the consumer needs whenever the live response supplies it or a live chain proves it. Never fabricate, infer, reorder, or substitute an apparent identifier. When sibling values must describe one selected result, anchor all of them to that same parsed producer record; never pair long or encoded strings merely because they occur nearby in the response. If useful core records legitimately lack the field, keep those records and their supported input classes, return the field as null/absent as the parser contract allows, and add a specific `workflow.limitations` entry naming the missing output and affected consumers. Required subkeys must be complete and non-empty whenever a field is emitted. The consumer may remain unverified or inconclusive; that gap alone must not make you discard valid producer results.
